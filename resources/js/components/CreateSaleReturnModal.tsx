...
export default function CreateSaleReturnModal({ open, onClose, sale }: CreateSaleReturnModalProps) {
  ...
  const calculateTotals = () => {
    let subtotal = 0;
    const items = sale.items.filter(item => (returnQuantities[item.id] || 0) > 0);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Sale Return - {sale.invoice_number}</DialogTitle>
        </DialogHeader>
